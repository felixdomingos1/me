...
const generateRealisticActivityData = (
  ...
  while (currentDate <= endDate) {
    ...
    const month = dataMonths[monthIndex] as keyof typeof monthData;
    ...
    const avgDaily = Math.max(1, Math.floor(monthTotal / daysInMonth));
    let activityCount = 0;

    const isWeekend = currentDate.getDay() === 0 || currentDate.getDay() === 6;

    if (isWeekend) {
      activityCount = Math.floor(Math.random() * (avgDaily + 3));
    } else {
      activityCount = Math.floor(Math.random() * (avgDaily * 2)) + Math.floor(avgDaily * 0.7);
    }
...
const getWeeksData = (
  ...
  const endDate = year === 2026 ? new Date(2026, 12, 31) : new Date(year, 11, 31);
...
export function ReadmeSection() {
  ...
  const activityData = useMemo(() => {
    return generateRealisticActivityData(selectedYear);
  }, [selectedYear]);

  const weeksData = useMemo(() => {
    return getWeeksData(activityData, selectedYear);
  }, [activityData, selectedYear]);

  const displayMonths = t.raw('github.months') as string[];

  const stacks = [
    {
      name: t('stacks.items.frontend.name'),
      icon: <Layout className="w-4 h-4 text-primary-neon" />,
      items: t('stacks.items.frontend.items'),
    },
    {
      name: t('stacks.items.backend.name'),
      icon: <Server className="w-4 h-4 text-accent-purple" />,
      items: t('stacks.items.backend.items'),
    },
    {
      name: t('stacks.items.mobile.name'),
      icon: <Smartphone className="w-4 h-4 text-primary-neon" />,
      items: t('stacks.items.mobile.items'),
    },
    {
      name: t('stacks.items.database.name'),
      icon: <Code className="w-4 h-4 text-accent-purple" />,
      items: t('stacks.items.database.items'),
    },
  ];

  const experiences = [
    {
      company: t('experience.roles.mobinads.company'),
      role: t('experience.roles.mobinads.role'),
      badge: t('experience.roles.mobinads.badge'),
    },
    {
      company: t('experience.roles.kiala.company'),
      role: t('experience.roles.kiala.role'),
      badge: t('experience.roles.kiala.badge'),
    },
    {
      company: t('experience.roles.digitalsindico.company'),
      role: t('experience.roles.digitalsindico.role'),
      badge: t('experience.roles.digitalsindico.badge'),
    },
    {
      company: t('experience.roles.fortytwo.company'),
      role: t('experience.roles.fortytwo.role'),
      badge: t('experience.roles.fortytwo.badge'),
    },
  ];
  ...
  const stats = [
    ...
    },
    {
      ...
      label: t('github.stats.repositories'),
      value: '45',
      change: '+3',
    },
    {
  ...
  const getYearNote = () => {
    if (selectedYear === 2026) {
      return t('github.until', { date: '08/05/2026' });
    }
    return '';
  };
      ...
      <div className="sticky top-0 z-50 flex items-center justify-between border-b border-white/10 bg-black/60 px-4 py-4 backdrop-blur-xl sm:px-5">
        ...
        <div className="flex items-center gap-3 overflow-hidden">
          ...
          <div className="flex min-w-0 items-center gap-2 font-mono text-[10px] sm:text-[11px]">
            ...
            <span className="truncate text-zinc-500">{t('header.path')}</span>
            <span className="truncate text-white">{t('header.filename')}</span>
          </div>
        </div>
      ...
      <div className="space-y-6 p-4 sm:space-y-8 sm:p-6 lg:p-8">
        ...
        <section>
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-white">{t('stacks.title')}</h3>
              <p className="text-xs text-zinc-500">{t('stacks.subtitle')}</p>
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {stacks.map((stack, i) => (
              ...
              >
                ...
                <p className="text-sm leading-relaxed text-zinc-400">{stack.items}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-white/10 bg-white/3 p-4 sm:p-6">
          <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-semibold text-white sm:text-base">{t('github.title')}</h3>
                <p className="text-[11px] text-zinc-500 sm:text-xs">{t('github.subtitle')}</p>
              </div>
            </div>
            ...
            {stats.map((stat, i) => (
              ...
              >
                <div className="mb-2 flex items-center justify-between">
                  <div className="text-primary-neon">{stat.icon}</div>
                  <span className="text-[10px] text-green-400">{stat.change}</span>
                </div>
                <p className="text-lg font-bold text-white">{stat.value}</p>
                <p className="mt-1 text-[10px] uppercase tracking-wider text-zinc-500">{stat.label}</p>
              </div>
            ))}
          </div>
            ...
            <div className="min-w-175 md:min-w-190">
              ...
              <div className="flex gap-xs sm:gap-0.75">
                {weeksData.map((week, weekIndex) => (
                  <div key={weekIndex} className="flex flex-col gap-xs sm:gap-0.75">
                    {week.map((day, dayIndex) => (
                      <div
                        ...
                        className={`
                          h-2.25 w-2.25 sm:h-2.75 sm:w-2.75 rounded-xs
                          ${getActivityColor(day.count)}
                          cursor-pointer transition-all duration-200 hover:scale-125 hover:ring-1 hover:ring-primary-neon
                        `}
                      />
                    ))}
                  </div>
              ...
              <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="text-[11px] font-medium text-primary-neon">
                  {getTotalContributions()} {t('github.contributionsIn', { year: selectedYear })}
                  {getYearNote()}
                </div>
              </div>
            </div>
        ...
        <section>
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-white">{t('experience.title')}</h3>
              <p className="text-xs text-zinc-500">{t('experience.subtitle')}</p>
            </div>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {experiences.map((exp, i) => (
              ...
              >
                ...
                <p className="mt-2 text-xs text-zinc-400">{exp.role}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section>
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">{t('achievements.subtitle')}</p>
            </div>
          </div>
        ...
        <section className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-xs text-zinc-500">felixsdemingos93@gmail.com</p>
            </div>
          </a>
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">+244 926 195 572</p>
            </div>
          </a>
        </section>
      </div>
